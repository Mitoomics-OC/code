...
def get_endmotifs(sample_path, base_num, target_path, end_total, MDS,MDS_path,
    ...
    for n in range(1,5):
      df_total[motif_set_n[n]] = df_total[motif_set_n[n]].apply(lambda x:x/x.sum(),axis=1) #20240710modified to compute ratio on the axis=1
    df_total.to_csv(os.path.join(end_outpath,"endmotifs_total_ratio.csv"), index=False)
